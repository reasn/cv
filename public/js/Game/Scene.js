...
ACV.Game.Scene.createFromData = function(element, data, performanceSettings) {
	...
	foreground = ACV.Game.Foreground.createFromData(data.foreground, performanceSettings);
...
ACV.Game.Scene.prototype.init = function(sceneDimensions) {
	...
	for ( var i in this.layers) {
		this.layers[i].init(this.element, this.prefs.dynamicViewport.minHeight,
			this.prefs.dynamicViewport.maxHeight);
		if (this.layers[i].prefs.justBehindPlayerLayer === true)
			this.foreground.init(this.layers[i].element, this.prefs.width,
				this.prefs.dynamicViewport.minHeight, this.prefs.dynamicViewport.maxHeight, this);
	}
